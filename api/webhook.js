...
app.post('/api/webhook', async (req, res) => {
  try {
    ...
    if (msg === '리셋' || msg === '@리셋' || msg === '탈출' || msg === '강제마을') {
      // 전투 상태만 초기화 (캐릭터 데이터 유지)
      u.phase = 'town';
      u.currentBattle = null;
      u.battleState = null;
      u.currentEnemy = null;
      u.inBattle = false;
      u.pendingDelete = false;  // 삭제 대기 상태도 해제
    ...
    // 삭제 확인 (2단계)
    if (msg === '삭제확인' && u.pendingDelete) {
      await deleteUser(userId);

      return res.json(reply(
        "🗑️ 캐릭터가 삭제되었습니다.\n\n새로운 모험을 시작하려면 아무 메시지나 보내주세요.",
        ['시작']
      ));
    }

    // 삭제 취소
    if (msg === '취소' && u.pendingDelete) {
      u.pendingDelete = false;
      await saveUser(userId, u);
    ...
    // 삭제 요청 (1단계 - 경고)
    if (msg === '초기화' || msg === '캐릭터삭제' || msg === '데이터삭제') {
      u.pendingDelete = true;
      await saveUser(userId, u);
